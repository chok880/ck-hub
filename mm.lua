-- โค้ด AutoFarm + FastAttack ปรับปรุงสมบูรณ์
        ...
        if MON then
            ...
            for _, target in pairs(Enemies:GetChildren()) do
                if target.Name == MON and target:FindFirstChild("Humanoid") and target.Humanoid.Health > 0 then
                    ...
                    if targetHRP then
                        Hum.CFrame = targetHRP.CFrame * CFrame.new(0, 5, 0)
                        task.wait(0.1)
                        pcall(function() RegHit:FireServer(targetHRP) end)
                        found = true
                    end
                end
            end

            -- ถ้าไม่มีมอนใกล้เคียง วาปไปตำแหน่ง MONPOS
            if not found then
                Hum.CFrame = MONPOS
            end
